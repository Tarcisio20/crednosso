...
driver = get_driver()
try:
    log_info("[BOOT-PY] >> ACESSANDO SITE")
    driver.get(SITE_URL)
    time.sleep(2)
    driver.maximize_window()
    time.sleep(2)
    log_info("[BOOT-PY] >> INSERINDO USUARIO")
    driver.find_element('xpath', '//*[@id="formulario:txLoginInput"]').send_keys(USERNAME)
    time.sleep(2)
    log_info("[BOOT-PY] >> INSERINDO SENHA")
    driver.find_element('xpath', '//*[@id="formulario:txSenhaInput"]').send_keys(PASSWORD)
    time.sleep(2)
    log_info("[BOOT-PY] >> ENTER PARA LOGIN NO SISTEMA")
    button = WebDriverWait(driver, 10).until(
        EC.presence_of_element_located((By.XPATH, '//*[@id="formulario:botaoEntrar"]'))
    )
    button.click()
    time.sleep(2)
    log_info("[BOOT-PY] >> ACESSANDO MENU")
    element = driver.find_element('xpath', '//*[@id="form-cabecalho:menuSistema"]/ul/li[5]')
    actions = ActionChains(driver)
    actions.move_to_element(element).perform()
    time.sleep(2)
    log_info("[BOOT-PY] >> ACESSANDO SUB MENU")
    ...
    subElement.click()
    time.sleep(2)
    log_info("[BOOT-PY] >> SELECIONANDO TIPO")
    ...
    type_select.click()
    time.sleep(2)
    log_info("[BOOT-PY] >> SELECIONANDO TIPO NO SELECT")
    type_sub_select = WebDriverWait(driver, 10).until(
        EC.presence_of_element_located((By.XPATH, '/html/body/div[6]/div[2]/ul/li[2]'))
    )
    actions.move_to_element(type_sub_select).perform()
    type_sub_select.click()
    time.sleep(2)
    log_info("[BOOT-PY] >> PESQUISAR ATMS")
    ...
    time.sleep(2)
    atms = driver.find_elements(By.CLASS_NAME, "divATM")
    time.sleep(2)
    infos = []
    num = 1
    for atm in atms:
        try:
            log_info("[BOOT-PY] >> PEGAR INFORMAÇÕES DE ATM ")
            ...
            log_info("[BOOT-PY] >> TERMINAL "+numero)
