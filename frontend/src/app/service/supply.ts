...
export const getSuppliesForDayPagination = async (data: DatePayload, page: number, pageSize: number) => {
  const token = Cookies.get('tokenSystemCredNosso')
  try {
    const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/supply/for-day-pagination`, data, {
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`
      },
...
export const add = async (data: supplyProps[]) => {
  const token = Cookies.get('tokenSystemCredNosso')
  try {
