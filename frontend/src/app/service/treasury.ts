...
export const getAllConditional = async () => {
  const token  =  Cookies.get('tokenSystemCredNosso')
  try{
    const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/treasury-conditional`, {
      headers: { 
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`
      }
...
export const del = async (id : number) => {
  try{
    const token  =  Cookies.get('tokenSystemCredNosso')
    const response = await  axios
    .get(`${process.env.NEXT_PUBLIC_API_URL}/treasury/del/${id}`, {
      headers: { 
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`
    },
