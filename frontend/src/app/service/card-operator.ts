...
export const getByIdTreasury = async (id: number) => {
  ...
  try {
    const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/card-operator/treasury/${id}`, {
      headers: {
...
export const getCardOperatorById = async (id: string) => {
  const token = Cookies.get('tokenSystemCredNosso')
  try {
    const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/card-operator/${id}`, {
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`
      }
...
export const add = async (data: cardOperatorType) => {
  try {
    ...
    const response = await axios
      .post(`${process.env.NEXT_PUBLIC_API_URL}/card-operator/add`, data, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`
        },
...
export const update = async (id : number , data : cardOperatorType) => {
  try{
    const token  =  Cookies.get('tokenSystemCredNosso')
    const response = await  axios
    .post(`${process.env.NEXT_PUBLIC_API_URL}/card-operator/update/${id}`, data, {
      headers: { 
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`
    },
