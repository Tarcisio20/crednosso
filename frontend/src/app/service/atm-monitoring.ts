...
export const getAtmMonitoring = async (date: string) => {
  const token = Cookies.get('tokenSystemCredNosso')
  try {
    const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/atm-monitoring/${date}`, {
      headers: {
...
export const updateAtmMonitoringManual = async () => {
  const token = Cookies.get('tokenSystemCredNosso')
  try {
    const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/atm-monitoring/update-manual`, {
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`
      }
...
export const getAtmMonitoringManualStatus = async () => {
  const token = Cookies.get('tokenSystemCredNosso')
  try {
    const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/atm-monitoring/update-manual/status`, {
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`
      }
