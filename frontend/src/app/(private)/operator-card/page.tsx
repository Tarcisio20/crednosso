...
export default function OperatorCard() {
  ...
  const search = async () => {
    ...
    if (allCardOperator.status === 300 || allCardOperator.status === 400 || allCardOperator.status === 500) {
      setError("Sem  contatos a listar")
      setLoading(false)
      return
    }
    const cardOperatortArry = [allCardOperator?.data.cardOperator]
    if (cardOperatortArry && cardOperatortArry[0]?.id) {
      ...
      <div className="flex flex-col gap-4 p-5 w-full">
        ...
        <div className='flex flex-row gap-2'>
          ...
          <div className='flex flex-col justify-end'>
            <Button color='#1E90FF ' onClick={search} secondaryColor='#87CEFA ' size='small' textColor='white' >Pesquisar</Button>
          </div>
        </div>
        ...
        <table className="flex-1 text-center p-3" width="100%">
          ...
          <tbody className=" text-xl">
            {cardOperators && cardOperators.map((item, index) => (
