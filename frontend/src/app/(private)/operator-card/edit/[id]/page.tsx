...
export default function OperationCardEdit() {
  ...
  const getById = async () => {
    ...
    if (cOperator.data.cardOperator.id) {
  ...
  const editCardOperator = async () => {
    ...
    let data = {
      ...
      status: statusOperatorCard === "0" ? false : true,
    };
  ...
  return (
    <Page>
      <TitlePages linkBack="/operator-card" icon={faPenToSquare}>
        Editar Cartão Operador
      </TitlePages>
      <div className="flex flex-col gap-4 p-5 w-full">
        ...
        <div className="flex flex-col gap-5">
          ...
          >
            <select
              className="w-full h-full m-0 p-0 text-white bg-transparent outline-none text-center text-lg"
              value={statusOperatorCard}
              onChange={(e) => setStatusOperatorCard(e.target.value)}
            >
