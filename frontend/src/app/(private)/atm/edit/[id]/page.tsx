...
export default function AtmEdit() {
  ...
  const updateAtm = async () => {
    ...
    setLoading(true)
    if (
      idSystemAtm === '' || !validateField(nameAtm) || !validateField(nameRedAtm) || idTreasutyAtm === '0' ||
      numStoreAtm === ''
    ) {
      ...
      <div className="flex flex-row gap-8 p-5 w-full">
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-5">
            ...
            <Input color="#DDDD" placeholder="Digite o Id no sistema" size="extra-large"
              value={idSystemAtm} onChange={(e) => setIdSystemAtm(e.target.value)} icon={faReceipt} />
          </div>
          <div className="flex flex-col gap-5">
            <label className="uppercase leading-3 font-bold">Nome</label>
            <Input color="#DDDD" placeholder="Digite o nome do Terminal" size="extra-large"
              value={nameAtm} onChange={(e) => setNameAtm(e.target.value)} icon={faLandmark} />
          </div>
          <div className="flex flex-col gap-5">
            <label className="uppercase leading-3 font-bold">Nome Reduzido</label>
            <Input color="#DDDD" placeholder="Digite o nome reduzido Terminal" size="extra-large"
              value={nameRedAtm} onChange={(e) => setNameRedAtm(e.target.value)} icon={faVault} />
          </div>
          <div className="flex flex-col gap-5">
            <label className="uppercase leading-3 font-bold">Nº da Loja</label>
            <Input color="#DDDD" placeholder="Digite o nome reduzido do Terminal" size="extra-large"
              value={numStoreAtm} onChange={(e) => setNumStoreAtm(e.target.value)} icon={faVault} />
          </div>
          <div className="flex flex-col gap-5">
